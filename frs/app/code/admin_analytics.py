...
@app.route("/admin_analytics/deadline_entry", methods=["POST", "GET"])
def admin_analytics_deadline_entry():
    ...
    if 'id' in session and session.get("user_type") == 'admin':
        ...
        cursor.execute('select course_details.admin_id,course_details.course_code,course_details.nfrs,'
                       'course_details.course_enroll_status,faculty_details.faculty_name from course_details right join '
                       'faculty_details on course_details.admin_id=faculty_details.admin_id where course_details.course_status="rejected" ')
        frs = cursor.fetchall()
        print(frs)
        return render_template('admin_analytics/frs_report.html', frs=frs, admin_name=admin_name, )
    else:
...
def admin_entry_course_change():
    ...
    if request.method == "POST":
        ...
        admin_id = request.form['admin'+index]#for admin id sivenas
        remarks = ''
        try:
        ...
        if check1 + check2 + check3 + check4 + check5 < 50:
            ...
            status = "approved"
            remarks = "Your Entry Approved"
        print(check1, check2, check3, check4, check5, remarks)
        cursor.execute(
            f'update course_details set course_status="{status}", course_enroll_status = "{remarks} ", nfrs = "{nfrs}" where course_id={course_id}')
        cursor.execute(f'update faculty_details set  nfrs = nfrs+ "{nfrs}" where admin_id={admin_id}')
        cursor.execute(f'select faculty_email from faculty_details where admin_id={admin_id} ')
        gmail=cursor.fetchall()
        gmail=gmail[0]['faculty_email']
        mysql.connection.commit()
#code to send mails
        MY_MAIL = "gokul.al21@bitsathy.ac.in"
        PASSWORD = "tynjordbkdcwsmwp"
        TO_MAIL = gmail
        ...
        connection.sendmail(from_addr=MY_MAIL, to_addrs=TO_MAIL,
                            msg=f"Subject:Your WIP portal Submission Action Summary -{status} \n\n Course Code : {course_code} \n Unit : {subject_id} \n LP :  {lesson_id} \n Remarks : {remarks} \n Negative FRS for this entry : {nfrs}")
        connection.close()
...
def admin_analytics_one_course():
    ...
    if 'id' in session and session.get("user_type") == 'admin':
        ...
        cursor.execute(f'SELECT * FROM course_details where course_code="{subject_id}" and course_status="pending"')
        course = cursor.fetchall()
        cursor.execute('SELECT * FROM subject')
        subject = cursor.fetchall()
...
def admin_entry_session_change():
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            ...
            [session_date, session_name, session_status, session_starttime, session_endtime, session_discription,
             session_id])
        mysql.connection.commit()
    return jsonify('success')
...
def admin_analytics_student():
    if 'id' in session and session.get("user_type") == 'admin':
        ...
        if school_id:
            ...
            return render_template('admin_analytics/details_table.html', student=student, count=count,
                                   admin_name=admin_name)
        cursor.execute(
            'SELECT * FROM student_details,school_details where  student_details.school_id =  school_details.school_id   order by student_name ASC')
        student = cursor.fetchall()
...
def admin_entry_student_select():
    ...
    if request.method == 'POST':
        ...
        print(student_id)
        cur.execute(
            "SELECT * FROM student_details,school_details where student_details.school_id=school_details.school_id and student_id = %s",
            [student_id])
        rsemployee = cur.fetchall()
        employeearray = []
        for rs in rsemployee:
...
def admin_entry_school_change():
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            'update school_details set school_name=%s, school_state = %s ,school_district=%s,school_pincode=%s,school_contact=%s,school_status=%s where school_id=%s',
            [school_name, school_state, school_district, school_pincode, school_contact, school_status, school_id])
        mysql.connection.commit()
    return jsonify('success')
